...
В сервере это решается функцией `_mean_center`: перед сравнением из эталонов вычитается их общий средний вектор. После этого NOUZ оценивает разницу между доменами: насколько один эталон выигрывает у остальных. Поэтому важны `spread`, проценты и пороги. Один raw cosine остаётся диагностикой, но не финальным решением.
